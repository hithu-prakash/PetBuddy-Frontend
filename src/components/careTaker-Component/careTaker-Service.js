...
function CareTakerList() {
  ...
  useEffect(() => {
    const fetchCareTakers = async () => {
      ...
      try {
        const response = await axios.get('/api/show-service', { params: { search } });
        const careTakersData = response.data
        // Check if the selected service is "Other" or if no caretakers match
        if (selectedService === 'Other' || careTakersData.length === 0 || !careTakersData.some(careTaker =>
            careTaker.serviceCharges.some(charge => charge.name === selectedService))) {
            navigate('/all-caretaker-v');
            return;
          }
       // Filter caretakers based on the selected service
       const filteredCareTakers = careTakersData.filter(careTaker =>
        careTaker.serviceCharges.some(charge => charge.name === selectedService)
      );
      ...
      setCareTakers(filteredCareTakers);
        ...
        const ratingsData = {};
        await Promise.all(filteredCareTakers.map(async (careTaker) => {
          try {
